...
def _read_nucleus_json(nucleus_path: str) -> dict:
    """Read and parse a JSON file directly from Nucleus (path under SERVER)."""
    result, _version, content = omni.client.read_file(_enc(f"{SERVER}{nucleus_path}"))
    if result != omni.client.Result.OK:
