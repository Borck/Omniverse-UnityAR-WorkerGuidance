...
def prepare_job(
    ...
    return {"job_id": job_id, "steps_synced": len(parts), "orphans_removed": removed}
...
def _cleanup_orphaned_versions(
    ...
    """
    live_versions = {s["assetVersion"] for s in manifest_steps}
    removed = 0
    for entry in asset_root.iterdir():
        ...
        try:
            ...
            # Don't fail the pipeline over a stuck file (could be locked by
            # FastAPI while serving an in-flight request, etc.).
