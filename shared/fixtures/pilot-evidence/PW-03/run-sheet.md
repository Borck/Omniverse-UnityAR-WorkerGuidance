...
## Run Metadata
- Workflow ID (`PW-01`..`PW-05`): PW-03
- Run ID: PW-03-RUN-001
- Date/time (UTC):
- Operator:
- Device model + OS:
- Unity build ID:
...
| 2 | network off during active step | frozen-step warning |  |  |  |
| 3 | network on | reconnect and state recovery |  |  |  |
| 4 | confirm post-recovery | deterministic next-step activation |  |  |  |
...
## Evidence Collected
- Diagnostics JSON path: `shared/fixtures/pilot-evidence/PW-03/diagnostics/`
- Screen recording path: `shared/fixtures/pilot-evidence/PW-03/recordings/`
- Server log extract path: `shared/fixtures/pilot-evidence/PW-03/server-logs/`
- Extra artifacts:
